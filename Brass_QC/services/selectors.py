...
def get_brass_qc_submitted_detail(lot_id):
    """
    Return read-only completed-history detail for the Brass QC Completed eye modal.

    Uses persisted submission snapshots and child lot history. It intentionally
    avoids live tray occupancy because completed-history trays may later be
    delinked and reused elsewhere.
    """
    ...
    if not submission:
        logger.warning("[BQC][submitted_detail] No completed submission for lot=%s", lot_id)
        return {"success": False, "error": f"No completed Brass QC record found for lot {lot_id}"}
    ...
    def _snapshot_trays(snapshot):
        ...
        for tray in raw_trays:
            ...
            if tray.get("reason_text"):
                item["reason_text"] = tray.get("reason_text")
            if item["tray_id"]:
                trays.append(item)
        return trays
    ...
    def _normalize_reasons(reasons):
        ...
        if reason_ids:
            reason_lookup = {
                str(obj.id): obj
                for obj in Brass_QC_Rejection_Table.objects.filter(id__in=reason_ids)
            }
            reason_lookup.update({
                str(obj.rejection_reason_id): obj
                for obj in Brass_QC_Rejection_Table.objects.filter(rejection_reason_id__in=reason_ids)
            })
    ...
    if not plating_stock_no and batch:
        plating_stock_no = getattr(batch, "plating_stk_no", "") or ""
    ...
    accept_children = list(
        BrassQC_PartialAcceptLot.objects
    ...
    reject_children = list(
        BrassQC_PartialRejectLot.objects
        .filter(parent_submission=submission)
        .order_by("created_at")
    )
    if submission.submission_type == "PARTIAL" and reject_children:
        ...
        reject_lots.append({
            ...
            "delinked_tray_ids": delinked_tray_ids,
            "delink_count": len(delinked_tray_ids),
            "remarks": submission.remarks or snapshot_data.get("remarks") or "",
            "created_at": _fmt_dt(submission.created_at),
        })
